...
class NLMAnalysis:
    ...
    def analyze_weights(self, similarity_map: np.ndarray) -> Dict[str, float]:
        ...
        percentiles = np.percentile(non_zero_weights, [25, 50, 75]) if len(non_zero_weights) > 0 else [0, 0, 0]
    ...
    def analyze_spatial_patterns(self, similarity_map: np.ndarray) -> Dict[str, float]:
        ...
        return {
            ...
            'edge_center_ratio': float(np.mean(similarity_map[0,:])/(similarity_map[center_y, center_x] + 1e-10)),
            'corner_center_ratio': float(similarity_map[0,0]/(similarity_map[center_y, center_x] + 1e-10)),
    ...
    def _compute_radial_decay(self, similarity_map: np.ndarray) -> float:
        ...
        center_y, center_x = similarity_map.shape[0]//2, similarity_map.shape[1]//2
        y_coords, x_coords = np.ogrid[:similarity_map.shape[0], :similarity_map.shape[1]]
        distances = np.sqrt((y_coords - center_y)**2 + (x_coords - center_x)**2)
        
        # Compute average weight at each unique distance
        unique_distances = np.unique(distances)
        avg_weights = [np.mean(similarity_map[distances == d]) for d in unique_distances]
    ...
    def render_analysis(self, img_array: np.ndarray, x: int, y: int, 
                       similarity_map: np.ndarray, search_range: List[Tuple[int, int]]) -> None:
        """Render complete NLM analysis."""
        ...
        with tabs[0]:
            self._render_coordinate_system(x, y, i, j, search_range)
        
        with tabs[1]:
            self._render_weight_distribution(similarity_map, weight_stats)
        
        with tabs[2]:
            self._render_search_region(img_array, similarity_map, x, y, search_range)
            
        with tabs[3]:
            self._render_spatial_analysis(similarity_map, spatial_stats)

    def _render_coordinate_system(self, x: int, y: int, i: int, j: int, 
    ...
    def _plot_cumulative_distribution(self, ax: plt.Axes, similarity_map: np.ndarray) -> None:
    ...
    def _render_search_region(self, img_array: np.ndarray, similarity_map: np.ndarray,
        ...
        with figure_context() as fig:
            ...
            ax1 = fig.add_subplot(gs[0])
            im1 = ax1.imshow(img_array, cmap=self.vis_config.colormap)
    ...
    def _render_spatial_analysis(self, similarity_map: np.ndarray, stats: Dict[str, float]) -> None:
    ...
    def _plot_radial_weight_distribution(self, similarity_map: np.ndarray) -> None:
        ...
        plt.plot(unique_distances, avg_weights, 'o-', label='Radial Weight Distribution')
        plt.xlabel('Distance')
    ...
    def _render_interpretation(self, weight_stats: Dict[str, float], spatial_stats: Dict[str, float]) -> None:
