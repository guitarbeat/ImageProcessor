...
@contextmanager
def display_context(settings: DisplaySettings, figsize: Optional[Tuple[int, int]] = None):
...
class ImageDisplay(Component):
    ...
    def render(self, image: Image.Image) -> None:
        ...
        try:
            ...
            with display_context(self.settings, figsize=self.config.figsize) as (fig, ax):
                ax.axis('off')
                ...
                if selected_pixel:
                    ...
                    if self.config.title == "Input Image":
                        ...
                        if st.session_state.get('filter_type') == 'nlm':
                            search_config = SearchWindowOverlayConfig.from_session_state()
                            search_size = None if st.session_state.get('use_full_image', True) else st.session_state.get('search_size', 21)
                            add_search_window_overlay(
                                ax=ax,
                                center=(x, y),
                ...
                with st.expander("Kernel View", expanded=True):
                    if selected_pixel:
                        x, y = selected_pixel
                        kernel = self._extract_kernel(x, y, img_array, self.kernel_config.kernel_size)
                        if kernel is not None:
                            self._render_kernel_view(kernel, x, y, self.kernel_config.kernel_size)

        except Exception as e:
            st.error(f"Error displaying image: {e}")

    def _extract_kernel(self, x: int, y: int, img_array: np.ndarray, kernel_size: int) -> Optional[np.ndarray]:
        """Extract kernel around selected pixel."""
        half = kernel_size // 2
        try:
            if (half <= x < img_array.shape[1] - half and
                half <= y < img_array.shape[0] - half):
                return img_array[y - half:y + half + 1, x - half:x + half + 1]
        except Exception as e:
            st.error(f"Error extracting kernel: {str(e)}")
        return None

    def _render_kernel_view(self, kernel: np.ndarray, x: int, y: int, kernel_size: int) -> None:
        ...
        add_kernel_overlay(
            ax=ax,
            center=(kernel_size // 2, kernel_size // 2),  # Center of the kernel view
            kernel_size=kernel_size,
