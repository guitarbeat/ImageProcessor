...
class ImageSelector(Component):
        ...
        if uploaded_file:
            try:
                ...
                st.session_state.current_image_id = (
                    f"{uploaded_file.name}_{hash(uploaded_file.getvalue())}"
                )
    ...
    def _render_sample_images(self) -> None:
        ...
        try:
            ...
            if not image_files:
                st.warning(
                    f"No sample images found in {self.config.sample_images_path}"
                )
                return
            ...
            for img_path in image_files:
                try:
                    thumb = self._create_thumbnail_cached(
                        str(img_path), self.config.thumbnail_size
                    )
                    images.append(thumb)
            ...
            if images:
                ...
                if selected_idx is not None and selected_idx != st.session_state.get(
                    "last_selected_idx"
                ):
                    ...
                    try:
                        ...
                        # Set unique image ID
                        st.session_state.current_image_id = (
                            f"{selected_path.name}_{hash(selected_path.read_bytes())}"
                        )
                        # Clear previous selections
                        st.session_state.selected_region = None
                        st.session_state.processed_regions = set()
