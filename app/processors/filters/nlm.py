...
class NLMComputation(FilterComputation):
    def __init__(self, kernel_size: int, filter_strength: float = 10.0, 
    ...
    def extract_patch(self, image: np.ndarray, x: int, y: int) -> Optional[np.ndarray]:
        ...
        try:
            if (half <= x < image.shape[1] - half and
    ...
    def compute_similarity_map(self, image: np.ndarray, x: int, y: int) -> np.ndarray:
        ...
        if self.search_window_size is None:
            ...
            similarity_map = np.zeros((height - 2*half_kernel, width - 2*half_kernel), dtype=np.float32)
        else:
        ...
        for i in range(search_area['y_start'], search_area['y_end']):
            for j in range(search_area['x_start'], search_area['x_end']):
                ...
                if comp_patch is not None:
                    ...
                    if (0 <= map_y < similarity_map.shape[0] and 
                        0 <= map_x < similarity_map.shape[1]):
    ...
    def compute(self, window: np.ndarray) -> float:
        ...
        try:
            ...
            st.error(f"Error in NLM computation: {e}")
            return float(window[window.shape[0]//2, window.shape[1]//2])
    ...
    def process_image(self, image: np.ndarray, progress_callback: Optional[Callable[[float], None]] = None) -> np.ndarray:
        ...
        for y in range(half, height - half):
            for x in range(half, width - half):
                ...
                if self.search_window_size is None:
                    search_range = [(half, height - half), (half, width - half)]
                else:
                    half_search = self.search_window_size // 2
                    search_range = [
                        (max(half, y - half_search), min(height - half, y + half_search + 1)),
                        (max(half, x - half_search), min(width - half, x + half_search + 1))
                    ]
    ...
    def compute_normalization_factors(self, image: np.ndarray, x: int, y: int) -> np.ndarray:
            ...
            for j in range(half, width - half):
                ...
                for ni in range(y_start, y_end):
                    for nj in range(x_start, x_end):
                        ...
                        comp_patch = image[ni-half:ni+half+1, nj-half:nj+half+1]
